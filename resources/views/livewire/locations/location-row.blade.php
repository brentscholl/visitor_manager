<li class="flex items-center justify-between gap-x-6 py-5 px-3 transition ease-in-out duration-75 hover:bg-blue-50 group">
    <a href="{{ route('locations.show', ['uuid' => $location->id]) }}" class="flex-grow cursor-pointer min-w-0 ">
        ...
        <div class="mt-1 flex items-center gap-x-2 leading-5 text-gray-500">
            <p class="whitespace-nowrap text-xs">Created on
                <time
                    datetime="{{ $location->created_at->format('Y-m-d') }}">{{ $location->created_at->format('F j, Y') }}</time>
            </p>
    ...
    <div class="flex items-center space-x-4">
        <a href="{{ route('locations.show', ['uuid' => $location->id]) }}" tooltip="Edit Form"
        ...
        @if($hasLayout)
            ...
            <a href="{{ route('locations.submissions', ['uuid' => $location->id]) }}" tooltip="View Submissions"
            ...
            <x-button btn="primary" wire:click="$set('showConnectModal', true)" class="tour__welcome--5 flex space-x-3 items-center"
                tooltip="Connect to tablet">
